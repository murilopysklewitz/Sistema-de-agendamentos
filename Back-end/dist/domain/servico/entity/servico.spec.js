...
describe('Servico', () => {
    ...
    describe('Constructor', () => {
        it('should create an instance with valid properties', () => {
            const servico = servico_1.Servico.with({ ...baseProps, id: 'valid-id' });
            expect(servico).toBeInstanceOf(servico_1.Servico);
            expect(servico.prop.id).toBe('valid-id');
        });
        it('should throw an error if nome is empty', () => {
            expect(() => servico_1.Servico.with({ ...baseProps, id: '123', nome: '' })).toThrow('Nome do serviço é obrigatório.');
        });
        it('should throw an error if preco is zero or negative', () => {
            expect(() => servico_1.Servico.with({ ...baseProps, id: '123', preco: 0 })).toThrow('Preço do serviço deve ser positivo.');
            expect(() => servico_1.Servico.with({ ...baseProps, id: '123', preco: -10 })).toThrow('Preço do serviço deve ser positivo.');
        });
        it('should throw an error if duracaoEmMinutos is zero or negative', () => {
            expect(() => servico_1.Servico.with({ ...baseProps, id: '123', duracaoEmMinutos: 0 })).toThrow('Duração do serviço deve ser um número positivo.');
            expect(() => servico_1.Servico.with({ ...baseProps, id: '123', duracaoEmMinutos: -10 })).toThrow('Duração do serviço deve ser um número positivo.');
        });
    });
    describe('Static create method', () => {
        it('should create a new service instance with a valid UUID', () => {
            const servico = servico_1.Servico.create(baseProps.nome, baseProps.preco, baseProps.descricao, baseProps.destaque, baseProps.duracaoEmMinutos);
            expect(servico).toBeInstanceOf(servico_1.Servico);
            const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
            expect(servico.id).toMatch(uuidRegex);
        });
        it('should throw an error on empty nome', () => {
            expect(() => servico_1.Servico.create('', 50, null, false, 60)).toThrow('Nome do serviço não pode ser vazio.');
        });
        it('should throw an error on invalid preco', () => {
            expect(() => servico_1.Servico.create('Serviço', 0, null, false, 60)).toThrow('Preço do serviço deve ser positivo.');
        });
    });
    describe('Update method', () => {
        let servico;
        beforeEach(() => {
            servico = servico_1.Servico.with({ ...baseProps, id: '123' });
        });
